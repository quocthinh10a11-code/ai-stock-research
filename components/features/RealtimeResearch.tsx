...
export function RealtimeResearch({ symbol }: { symbol: string }) {
  ...
  return <section className="panel overflow-hidden">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 md:px-6"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-navy text-white"><Sparkles size={17}/></span><div><h2 className="font-bold text-navy">Phân tích AI có dẫn nguồn</h2><p className="mt-0.5 text-xs text-slate-500">{new Date(report.asOf).toLocaleString("vi-VN")} · {report.cached ? "cache 15 phút" : "tìm kiếm realtime"} · {report.model}</p></div></div><button onClick={refresh} className="btn-secondary"><RefreshCw size={15}/>Làm mới</button></header>
