...
export function RealtimeResearch({ symbol }: { symbol: string }) {
  ...
  return <section className="panel overflow-hidden">
    ...
    {report.staleFallback && <div className="flex gap-3 border-b border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900"><AlertTriangle className="mt-0.5 shrink-0" size={18}/><div><p className="font-semibold">Live agent chưa hoàn tất; đang hiển thị báo cáo cache cũ.</p><p className="mt-1 text-xs leading-5">{report.fallbackReason ?? "Nguồn Internet hoặc AI tạm thời không khả dụng."}</p></div></div>}
